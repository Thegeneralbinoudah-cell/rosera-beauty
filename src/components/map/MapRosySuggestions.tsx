      ...
      <div className="mb-1.5 flex items-center gap-1.5 px-0.5">
        ...
        {activeId ? (
          <button
            type="button"
            onClick={onClear}
            className="ms-auto text-[10px] font-semibold text-foreground underline-offset-2 hover:text-foreground hover:underline sm:text-[11px]"
          >
            {lang === 'ar' ? 'إلغاء' : 'Clear'}
          </button>
        ) : null}
        ...
        {suggestions.map((s) => {
          ...
          return (
            <button
              ...
              onClick={() => onSelect(s)}
              className={cn(
                'min-h-[32px] shrink-0 rounded-full border-2 px-3 py-1.5 text-[11px] font-bold transition-all duration-200 ease-out sm:text-[12px]',
                active
                  ? 'gradient-rosera border-transparent text-white shadow-elevated'
                  : 'border-primary/20 bg-card/95 text-foreground shadow-sm hover:border-primary/35 hover:bg-accent/60 hover:text-foreground dark:bg-card/90'
              )}
            >
