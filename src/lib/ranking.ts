...
import type { BoostContext, BoostMeta } from '@/lib/boosts'

/** Minimal skin context for ranking (same shape as analysis_result JSON). */
...
/** Aggregated behavior for scoring (bookings, orders, favorites, decayed events). */
...
export type ScoredItem<T> = {
  ...
  score: number
  reasons: string[]
  /** Paid / sponsored uplift applied (transparent in UI). */
  sponsored?: boolean
...
/** Hard cap on sponsored points so organic quality still dominates. */
...
function sponsoredDelta(baseScore: number, meta: BoostMeta | undefined): number {
  ...
  const raw = (baseScore * meta.pct) / 100
  return Math.min(MAX_SPONSORED_POINTS, raw)
}

/** Tokens from skin analysis for text overlap. */
...
): Pick<ScoredItem<unknown>, 'score' | 'reasons' | 'sponsored' | 'sponsorLabel'> {
  const add = sponsoredDelta(baseScore, meta)
  if (add <= 0) {
    return { score: baseScore, reasons, sponsored: false }
  }
